...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider, FlutterWebRTCPlugin.RecEvent {
    ...
    @Override
    public void onMuxed(String path) {
        Log.i(TAG, "on muxed:" + path);
        methodChannel.invokeMethod("onMuxed", Collections.singletonMap("path", path));
    }

    @Override
    public void onRestartRec() {
        getActivity().runOnUiThread(() -> methodChannel.invokeMethod("restartRec", null));
    }
        ...
        render.setId((int) entry.id());
            ...
            case "startRecordToFile": {
                ...
                if (videoTrackId != null) {
                    ...
                    if (track instanceof VideoTrack) videoTrack = (VideoTrack) track;


                }
                recorderId = call.argument("recorderId");
                Log.i(TAG, "start rec id:" + recorderId);
                if (videoTrack != null) {
                    Log.i(TAG, "path:" + path);
                    try {
                        getUserMediaImpl.startRecordingToFile(path, recorderId, videoTrack);
                    } catch (Exception e) {
            ...
            case "stopRecordToFile": {
                recorderId = call.argument("recorderId");
                Log.i(TAG, "stop rec id:" + recorderId + ", path:" + path);
            ...
            case "captureFrame": {
                String capPath = call.argument("path");
                String videoTrackId = call.argument("trackId");
                String peerConnectionId = call.argument("peerConnectionId");
                if (videoTrackId != null) {
                    MediaStreamTrack track = getTrackForId(videoTrackId, peerConnectionId);
                    if (track instanceof VideoTrack) new FrameCapturer((VideoTrack) track, new File(capPath), result);
                    else resultError("captureFrame", "It's not video track", result);
                } else {
